...
<!--    Carousel Slider    -->
        ...
        <div class="carousel-item active">
          ...
          <div class="carousel-caption">
           <p>{{ nouvelleslider.titre }}</p>
              ...
              <div class="nouvelles-left container-fluid" style="padding:30px 15px">
                <div class="row">
                  <div class="col-md-9">
                    <hr>
                    ...
                    <div class="container-fluid">
                    <div class="row">
                      <div v-for="nouvelle in nouvelles.data" class="col-md-4 carde text-right">
                        <div class="image">
                          <router-link v-bind:to="'/nouvelle/' + nouvellessliderleftactive.slug"><img :src="'storage/'+nouvelle.photo" height="241px" width="100%"></router-link>
                        </div>
                        <div class="content">
                          
                          <h2>{{ nouvelle.titre }}</h2>
                          <p style="font-size: 0.8em;">{{ nouvelle.created_at }}</p>
                          <p>{{ nouvelle.p_text | truncate(200) }}</p>
                        </div>
                      </div>
                    </div>
                      <pagination :data="nouvelles" @pagination-change-page="getResults"></pagination> 
                    </div>
                  </div>
                  <div class="col-md-3 social">
                    ...
                    <hr>
                    <iframe width="100%" height="315" src="https://www.youtube-nocookie.com/embed/dIh3izNLrc4" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
                    <hr>
                    <div class="title">Réseau social</div>
                    <hr>
                    <div class="login-box">
      ...
      <a href="#" class="social-button" id="linkedin-connect"> <span>Connect with Instagram</span></a>
    </div>
    <hr>
                    <div class="title">Page Facebook</div>
                    <hr>

    <iframe src="https://www.facebook.com/plugins/page.php?href=https%3A%2F%2Fwww.facebook.com%2FAsfartv1958%2F&tabs=timeline&width=480&height=500&small_header=false&adapt_container_width=true&hide_cover=false&show_facepile=true&appId" width="100%" height="500" style="border:none;overflow:hidden" scrolling="no" frameborder="0" allowTransparency="true" allow="encrypted-media"></iframe>
...
<script>
  ...
  mounted() {
    ...
    methods: {
      getResults(page = 1) {
      axios.get('api/nouvelles?page=' + page)
        .then(response => {
          this.nouvelles = response.data;
        });
    },
      getnouvelles() {
        axios.get('http://localhost:8000/api/nouvelles')
            .then(res => {
                this.nouvelles = res.data;
...
.nouvelles-left .carde {
    box-shadow: 0 4px 8px 0 rgba(0, 0, 0, 0.2), 0 6px 20px 0 rgba(0, 0, 0, 0.19);
    padding: 23px 6px;
    margin-bottom: 20px;
}
.nouvelles-left .carde .content h2,.nouvelles-left .carde .content p {
  font-family: 'Tajawal', sans-serif;
}
...
.social #facebook-connect:hover span {
    color: #FFF;
    -webkit-transition: all .3s ease;
    -moz-transition: all .3s ease;
    -ms-transition: all .3s ease;
...
.social #google-connect:hover span {
    color: #FFF;
    -webkit-transition: all .3s ease;
    -moz-transition: all .3s ease;
    -ms-transition: all .3s ease;
...
.social #twitter-connect:hover span {
    color: #FFF;
    -webkit-transition: all .3s ease;
    -moz-transition: all .3s ease;
    -ms-transition: all .3s ease;
...
.social #linkedin-connect:hover span {
    color: #FFF;
    -webkit-transition: all .3s ease;
    -moz-transition: all .3s ease;
    -ms-transition: all .3s ease;
